...
function BackBar({ onBack, title, subtitle, right }) {
  return (
    <div style={{ display:"flex", alignItems:"center", padding:"16px 20px", gap:12 }}>
      <button onClick={onBack} style={{ width:36, height:36, borderRadius:100, background:C.card, border:"none", cursor:"pointer", color:C.textMid, fontSize:18, display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 1px 6px rgba(0,0,0,0.08)" }}>←</button>
      <div style={{ flex:1 }}>
        <div style={{ fontSize:17, fontWeight:700, color:C.text }}>{title}</div>
        {subtitle && <div style={{ fontSize:13, color:C.textLight, marginTop:1 }}>{subtitle}</div>}
      </div>
      {right}
    </div>
  );
    ...
    <div style={{ padding:"0 16px 40px" }}>
      ...
      <div style={{ ...card(), overflow:"hidden", marginBottom:16, position:"relative" }}>
        {img
          ...
          : <div style={{ height:180, display:"flex", alignItems:"center", justifyContent:"center", fontSize:60, background:C.primaryLight }}>📦</div>
        }
        <div style={{ position:"absolute", bottom:12, right:12 }}>
          {recognizing
            ? <div className="shimmer" style={{ width:52, height:52, borderRadius:14 }} />
            : <div className="pop" style={{ width:52, height:52, borderRadius:14, background:sticker?.bg||C.primaryLight, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", fontSize:25, border:`2px solid ${sticker?.color||C.primary}`, boxShadow:"0 2px 10px rgba(0,0,0,0.18)" }}>
                {sticker?.emoji||"📦"}
                <div style={{ fontSize:8, color:C.primary, marginTop:1 }}>{sticker?.name}</div>
              </div>
          }
        </div>
        {recognizing && (
          <div style={{ position:"absolute", bottom:18, left:14, fontSize:13, color:"#fff", background:"rgba(26,37,53,0.45)", borderRadius:8, padding:"3px 9px" }}>
      ...
      <div style={{ ...card(), padding:"20px 18px", display:"flex", flexDirection:"column", gap:18 }}>
        <div>
          <div style={{ fontSize:12, color:C.textLight, letterSpacing:1.5, marginBottom:8, textTransform:"uppercase" }}>物品名称</div>
          <input
            value={form.name}
            onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
            placeholder={recognizing ? "识别完成后自动填入…" : "输入名称"}
            style={{ width:"100%", padding:"11px 13px", borderRadius:12, border:"1.5px solid #E4ECF4", background:"#F6F9FC", fontSize:17, color:C.text, fontFamily:"inherit", outline:"none" }}
          />
        </div>

        <div>
          <div style={{ fontSize:12, color:C.textLight, letterSpacing:1.5, marginBottom:8, textTransform:"uppercase" }}>丢弃途径</div>
          <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
            {DISPOSE_METHODS.map(m => (
              <button key={m} className="press" onClick={() => setForm(f => ({ ...f, method: m }))} style={{
                padding:"8px 13px", borderRadius:100, fontSize:14, cursor:"pointer", fontFamily:"inherit", fontWeight:600, transition:"all 0.15s",
        ...
        <div>
          <div style={{ fontSize:12, color:C.textLight, letterSpacing:1.5, marginBottom:8, textTransform:"uppercase" }}>所得收入 (¥)</div>
          <input
            type="number"
            value={form.income}
        ...
        <button className="press" onClick={onSave} style={{
          ...
          fontSize:17, fontWeight:700, cursor:"pointer", opacity: recognizing ? 0.65 : 1,
    ...
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"HuiWen,'Songti SC',Georgia,serif", color:C.text }}>
      ...
      <div style={{ padding:"20px 20px 24px", background:C.bg }}>
        <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between" }}>
          <div>
            <div style={{ fontSize:13, color:C.textLight, letterSpacing:2, textTransform:"uppercase", marginBottom:3 }}>当断则断   该扔就扔</div>
            <div style={{ fontSize:30, fontWeight:800, color:C.text, letterSpacing:-0.5 }}>每日一轻</div>
          </div>
          <button className="press" onClick={()=>setView(view==="summary"?"calendar":"summary")} style={{
            ...card(), border:"none", padding:"8px 16px", fontSize:14, fontWeight:700,
        ...
        <div className="fade" style={{ paddingBottom:80 }}>
          ...
          <div style={{ background:C.card, boxShadow:"0 2px 16px rgba(74,130,196,0.10)", borderRadius:40, padding:"16px 16px 18px" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
              <button onClick={prevMonth} className="press" style={{ width:34, height:34, border:"none", fontSize:21, color:C.textMid, display:"flex", alignItems:"center", justifyContent:"center", borderRadius:100, background:"#F0F5FA", cursor:"pointer" }}>‹</button>
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:23, fontWeight:800, color:C.text }}>{MONTHS_CN[month]}</div>
                <div style={{ fontSize:13, color:C.textLight, letterSpacing:2 }}>{year}</div>
              </div>
              <button onClick={nextMonth} className="press" style={{ width:34, height:34, border:"none", fontSize:21, color:C.textMid, display:"flex", alignItems:"center", justifyContent:"center", borderRadius:100, background:"#F0F5FA", cursor:"pointer" }}>›</button>
            </div>

            <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", marginBottom:8 }}>
              {WEEKDAYS.map(d=>(
                <div key={d} style={{ textAlign:"center", fontSize:12, fontWeight:700, color:C.textLight, letterSpacing:0.8, textTransform:"uppercase" }}>{d}</div>
              ))}
            </div>
              ...
              {Array(getDaysInMonth(year,month)).fill(null).map((_,i)=>{
                ...
                return (
                  <div key={day} className="press" onClick={()=>!isFuture&&handleDayClick(day)} style={{
                    ...
                    {count>0 && !first?.image && <div style={{ width:"100%", height:"100%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18 }}>{first.emoji}</div>}
                    {count===0 && <div style={{ width:"100%", height:"100%", display:"flex", alignItems:"center", justifyContent:"center" }}>
                      {!isToday && <span style={{ fontSize:14, color:C.textMid }}>{day}</span>}
                    </div>}
                    {count>1 && <div style={{ position:"absolute", top:2, right:2, background:"rgba(26,37,53,0.6)", color:"#fff", fontSize:8, borderRadius:5, padding:"1px 3px", fontWeight:700 }}>{count}</div>}
                    {isToday && (
                      <>
                        ...
                        <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", pointerEvents:"none" }}>
                          <span style={{ fontSize:14, color:"#434951", fontWeight:800 }}>{day}</span>
                        </div>
                      </>
                    )}
                  </div>
            ...
            {[
              ...
              <div key={i} style={{ display:"flex", flex:1 }}>
                <div style={{ flex:1, textAlign:"center" }}>
                  <div style={{ fontSize:25, fontWeight:800, color:C.text }}>{s.val}</div>
                  <div style={{ fontSize:12, color:C.textLight, marginTop:2, letterSpacing:0.3 }}>{s.label}</div>
                </div>
      ...
      {view==="dayList" && selDate && (
        <div className="fade" style={{ paddingBottom:60 }}>
          <BackBar
            ...
            right={
              <button className="press" onClick={()=>openAdd(selDate)} style={{ ...card(), border:"none", padding:"8px 16px", fontSize:15, fontWeight:700, color:C.primary, cursor:"pointer" }}>+ 新增</button>
            }
          />
          <div style={{ padding:"4px 16px", display:"flex", flexDirection:"column", gap:10 }}>
            {getDateItems(selDate.key).length===0 ? (
              <div style={{ textAlign:"center", padding:"60px 20px", color:C.textLight }}>
                <div style={{ fontSize:41, marginBottom:10 }}>🌤️</div>
                <div style={{ fontSize:16, marginBottom:16 }}>今天还没有记录</div>
                <button className="press" onClick={()=>openAdd(selDate)} style={{ background:C.primary, color:"#fff", border:"none", borderRadius:100, padding:"10px 28px", fontSize:16, fontWeight:700, boxShadow:"0 4px 14px rgba(74,144,196,0.35)" }}>立即记录</button>
              </div>
              ...
              <div key={item.id} className="press" onClick={()=>{setSelItem(item);setView("detail");}} style={{ ...card(), display:"flex", alignItems:"center", overflow:"hidden" }}>
                ...
                <div style={{ flex:1, padding:"0 14px", minWidth:0 }}>
                  <div style={{ fontSize:17, fontWeight:700, color:C.text }}>{item.name}</div>
                  <div style={{ fontSize:13, color:C.textLight, marginTop:3 }}>{item.method}</div>
                </div>
                {item.income>0 && <div style={{ fontSize:16, fontWeight:800, color:C.primary, paddingRight:8 }}>+¥{item.income}</div>}
                <div style={{ color:C.textLight, fontSize:18, paddingRight:14 }}>›</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {view==="detail" && selItem && (
        <div className="fade" style={{ paddingBottom:60 }}>
          <BackBar
            ...
            title="物品详情"
            right={<button className="press" onClick={()=>openEdit(selItem)} style={{ ...card(), border:"none", padding:"8px 16px", fontSize:15, fontWeight:700, color:C.textMid, cursor:"pointer" }}>编辑</button>}
          />
            ...
            <div style={{ width:"100%", height:240, background:C.primaryLight, position:"relative" }}>
              {selItem.image
                ...
                : <div style={{ width:"100%", height:"100%", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:8 }}>
                    <div style={{ fontSize:69 }}>{selItem.emoji}</div>
                    <div style={{ fontSize:14, color:C.textLight }}>暂无照片</div>
                  </div>
              }
              <div style={{ position:"absolute", bottom:14, right:14, width:56, height:56, borderRadius:14, background:selItem.bg||"rgba(255,255,255,0.95)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", fontSize:28, border:`2px solid ${selItem.color||C.primary}`, boxShadow:"0 2px 10px rgba(0,0,0,0.15)" }}>
                {selItem.emoji}
                <div style={{ fontSize:8, color:C.primary, marginTop:1 }}>{selItem.name}</div>
              </div>
            </div>

            <div style={{ padding:"18px 18px 20px" }}>
              <div style={{ fontSize:25, fontWeight:800, color:C.text, marginBottom:2 }}>{selItem.name}</div>
              <div style={{ fontSize:13, color:C.textLight, marginBottom:18 }}>AI 生成贴纸</div>
              ...
              {[
                ...
                <div key={row.label} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"13px 0", borderBottom:"1px solid #F0F4F8" }}>
                  <div style={{ fontSize:15, color:C.textMid }}>{row.label}</div>
                  <div style={{ fontSize:16, fontWeight:700, color:row.label==="所得收入"&&selItem.income>0?C.primary:C.text }}>{row.value}</div>
                </div>
              ))}

              <button className="press" onClick={()=>deleteItem(selItem)} style={{ width:"100%", marginTop:22, padding:"13px", background:"#FFF0F0", color:C.danger, border:"none", borderRadius:100, fontSize:17, fontWeight:700 }}>
                删除此记录
              </button>
            </div>
          </div>
          ...
          {addStep==="photo" && (
            <div style={{ padding:"8px 16px 24px" }}>
              <div style={{ ...card(), padding:"52px 20px", textAlign:"center", marginBottom:16 }}>
                ...
                <div style={{ fontSize:17, color:C.textMid, marginBottom:4 }}>拍下你要丢弃的物品</div>
                <div style={{ fontSize:13, color:C.textLight }}>AI 将自动识别并生成贴纸</div>
              </div>
              <div style={{ display:"flex", gap:12 }}>
                <button className="press" onClick={()=>cameraRef.current.click()} style={{ flex:1, padding:"14px 0", borderRadius:100, background:C.primary, color:"#fff", border:"none", fontSize:17, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center", gap:8, boxShadow:"0 4px 14px rgba(74,144,196,0.35)" }}>📷 拍照</button>
                <button className="press" onClick={()=>galleryRef.current.click()} style={{ flex:1, padding:"14px 0", borderRadius:100, background:C.card, color:C.primary, border:"none", fontSize:17, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center", gap:8, boxShadow:"0 2px 10px rgba(0,0,0,0.08)" }}>🖼️ 相册</button>
              </div>
            </div>
          )}
          ...
          <FormPanel img={img} sticker={sticker} recognizing={recognizing} form={form} setForm={setForm} onSave={saveEdit} saveLabel="保存修改 ✓"
            extraTop={
              <div style={{ display:"flex", gap:10, marginBottom:16 }}>
                <button className="press" onClick={()=>cameraRef.current.click()} style={{ flex:1, padding:"10px", borderRadius:100, background:C.card, color:C.primary, border:"none", fontSize:15, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center", gap:6, boxShadow:"0 1px 6px rgba(0,0,0,0.08)" }}>📷 重新拍照</button>
                <button className="press" onClick={()=>galleryRef.current.click()} style={{ flex:1, padding:"10px", borderRadius:100, background:C.card, color:C.primary, border:"none", fontSize:15, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center", gap:6, boxShadow:"0 1px 6px rgba(0,0,0,0.08)" }}>🖼️ 换图</button>
              </div>
            }
          />
        </div>
      ...
      {view==="summary" && (
        <div className="fade" style={{ padding:"16px 16px 60px" }}>
          ...
          <div style={{ background:"#434951", borderRadius:24, padding:"28px 24px", color:"#fff", marginBottom:14 }}>
            <div style={{ fontSize:12, letterSpacing:2.5, opacity:0.75, marginBottom:16, textTransform:"uppercase" }}>断舍离成就</div>
            <div style={{ display:"flex", justifyContent:"space-around" }}>
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:53, fontWeight:800, lineHeight:1 }}>{allItems.length}</div>
                <div style={{ fontSize:13, opacity:0.8, marginTop:4 }}>累计丢弃</div>
              </div>
              <div style={{ width:1, background:"rgba(255,255,255,0.25)" }}/>
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:53, fontWeight:800, lineHeight:1 }}>¥{totalIncome.toFixed(0)}</div>
                <div style={{ fontSize:13, opacity:0.8, marginTop:4 }}>累计收入</div>
              </div>
            </div>
          </div>
          ...
          {allItems.length>0 && (
            <div style={{ ...card(), padding:"16px", marginBottom:14 }}>
              <div style={{ fontSize:12, color:C.textLight, letterSpacing:1.5, marginBottom:12, textTransform:"uppercase" }}>所有贴纸</div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                {allItems.map((item,i)=>(
                  item.image
                    ...
                    : <div key={i} style={{ width:50, height:50, borderRadius:12, background:item.bg||C.primaryLight, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", border:`2px solid ${item.color||C.primary}`, fontSize:23 }}>
                        {item.emoji}
                        <div style={{ fontSize:8, color:C.primary, marginTop:1 }}>{item.name}</div>
                      </div>
                ))}
              </div>
            </div>
          ...
          {allItems.length===0 ? (
            <div style={{ textAlign:"center", padding:"60px 20px", color:C.textLight }}>
              ...
              <div style={{ fontSize:16 }}>还没有记录</div>
              <div style={{ fontSize:13, marginTop:6 }}>从日历页面开始你的第一次断舍离</div>
            </div>
          ) : (
            <>
              <div style={{ fontSize:12, color:C.textLight, letterSpacing:1.5, marginBottom:10, textTransform:"uppercase" }}>记录明细</div>
                  ...
                  return (
                    <div key={i} className="press" onClick={()=>{ setSelDate({year:parseInt(parts[0]),month:parseInt(parts[1])-1,day:parseInt(parts[2]),key:item.date}); setSelItem(item); setView("detail"); }} style={{ ...card(), display:"flex", alignItems:"center", overflow:"hidden" }}>
                      <div style={{ width:58, height:58, flexShrink:0, background:item.bg||C.primaryLight, overflow:"hidden" }}>
                        {item.image ? <img src={item.image} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }} />
                          : <div style={{ width:"100%", height:"100%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:25 }}>{item.emoji}</div>}
                      </div>
                      <div style={{ flex:1, padding:"0 12px", minWidth:0 }}>
                        <div style={{ fontSize:16, fontWeight:700 }}>{item.name}</div>
                        <div style={{ fontSize:13, color:C.textLight, marginTop:2 }}>{item.date} · {item.method}</div>
                      </div>
                      {item.income>0 && <div style={{ fontSize:16, fontWeight:800, color:C.primary, paddingRight:10 }}>+¥{item.income}</div>}
                    </div>
                  );
                })}
              </div>
