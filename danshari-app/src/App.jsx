...
export default function App() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"-apple-system,'PingFang SC','Helvetica Neue',sans-serif", color:C.text }}>
      <style>{`
                    ...
                    {isToday && (
                      <>
                        <svg viewBox="0 0 90 94" style={{ position:"absolute", inset:0, width:"100%", height:"100%", pointerEvents:"none" }}>
                          <path d={STAR_PATH} fill="#EEF4FB" stroke="#434951"/>
                        </svg>
          ...
          {allItems.length>0 && (
            <div style={{ ...card(), padding:"16px", marginBottom:14 }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                {allItems.map((item,i)=>(
                  item.image
                    ? <div key={i} style={{ width:50, height:50, flexShrink:0 }}>
                        <svg viewBox="0 0 90 94" style={{ width:"100%", height:"100%" }}>
                          <defs><clipPath id={`sc${i}`}><path d={STAR_PATH}/></clipPath></defs>
                          <image href={item.image} x="0" y="0" width="90" height="94" clipPath={`url(#sc${i})`} preserveAspectRatio="xMidYMid slice"/>
                          <path d={STAR_PATH} fill="none" stroke={item.color||C.primary} strokeWidth="1"/>
                        </svg>
                      </div>
                    : <div key={i} style={{ width:50, height:50 }}>
                        <svg viewBox="0 0 90 94" style={{ width:"100%", height:"100%" }}>
                          <path d={STAR_PATH} fill={item.bg||C.primaryLight} stroke={item.color||C.primary} strokeWidth="1"/>
                  ...
                  return (
                    <div key={i} className="press" onClick={()=>{ setSelDate({year:parseInt(parts[0]),month:parseInt(parts[1])-1,day:parseInt(parts[2]),key:item.date}); setSelItem(item); setView("detail"); }} style={{ ...card(), display:"flex", alignItems:"center", padding:"16px" }}>
                      <div style={{ width:40, height:40, flexShrink:0 }}>
                        <svg viewBox="0 0 90 94" style={{ width:"100%", height:"100%" }}>
                          <defs><clipPath id={`rl${i}`}><path d={STAR_PATH}/></clipPath></defs>
                          {item.image
                            ? <image href={item.image} x="0" y="0" width="90" height="94" clipPath={`url(#rl${i})`} preserveAspectRatio="xMidYMid slice"/>
                            : <><path d={STAR_PATH} fill={item.bg||C.primaryLight}/><text x="45" y="52" textAnchor="middle" dominantBaseline="middle" fontSize="30">{item.emoji}</text></>
                          }
                          <path d={STAR_PATH} fill="none" stroke={item.color||C.primary} strokeWidth="1"/>
                        </svg>
                      </div>
